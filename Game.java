...
public class Game implements Runnable {
	...
	private static final int EXTRA_LIFE = 500; //every 500 points, the player gains one life.
	            ...
	            for (int j = 0; j < blocks.length; j++) { //Loop through each row of blocks
	            	for (int k = 0; k < blocks[j].length; k++) { //Loop through each column of blocks
	            		...
	            		if (!block.blockWasHit()) {
		                    ...
		                    if (touches(r1, r2)) {
		    	            	//changeBallAngle(ball1, block); didnt think this was necessary for the blocks itself.
